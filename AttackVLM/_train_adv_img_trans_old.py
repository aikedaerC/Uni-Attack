...
if __name__ == "__main__":
    ...
    imagenet_data    = ImageFolderWithPaths(args.cle_data_path, transform=transform_fn)
    target_data   = ImageFolderWithPaths(args.tgt_data_path, transform=transform_fn)
    repeat_count = len(imagenet_data) // len(target_data) + 1
    ...
    balanced_target_data = Subset(extended_target_data, range(len(imagenet_data)))
    
    data_loader_imagenet = torch.utils.data.DataLoader(imagenet_data, batch_size=args.batch_size, shuffle=False, num_workers=0, drop_last=False)
    ...
    # start attack
    args.num_samples = len(imagenet_data)
