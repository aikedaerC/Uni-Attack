...
# python benchmark/evaluate.py --predictions_path /home/aikedaer/mydata/data/comp/noise --answer_path /home/aikedaer/mydata/data/comp/
def to_str(e):
    return str(e)

@click.command()
@click.option('--predictions_path', help='Path to predictions file.')
@click.option('--answer_path', help='Path to asw file.')
@click.option('--batch_size', help='Batch size for evaluation.', default=1)
def evaluate(predictions_path: str, answer_path: str, batch_size: int) -> float:
    """
    ...
    """
    references = pd.read_csv(answer_path)
    # import pdb;pdb.set_trace()
    references = references[[Keys.question_id, Keys.segment_id, Keys.answer]]
    references = references.groupby([Keys.question_id, Keys.segment_id]).agg(list) # , Keys.question
